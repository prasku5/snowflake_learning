...
CREATE WAREHOUSE COMPUTE_WH_SA
WITH 
    WAREHOUSE_SIZE = 'XSMALL' -- Choose size (XSMALL, SMALL, MEDIUM, LARGE, XLARGE, etc.)
    AUTO_SUSPEND = 300        -- Automatically suspend warehouse after 300 seconds of inactivity
    AUTO_RESUME = TRUE        -- Automatically resume warehouse when a query is executed
    INITIALLY_SUSPENDED = False; -- Warehouse starts in a suspended state
...
select GRADER(step, (actual = expected), actual, expected, description) as graded_results from (
 ...
 ,( select count(*) 
   from cINFORMATION_SCHEMA.TABLES 
...
where query_text like 'execute notebook%Uncle Yer\'s Root Depth Notebook%''
-- DO NOT EDIT ANYTHING BELOW THIS LINE. THE CODE MUST BE RUN EXACTLY AS IT IS WRITTEN
--Remember that every time you run a DORA check, the context needs to be set to the below settings. 
use database UTIL_DB;
use schema PUBLIC;
...
// You dont have to list anything for the
...
CREATE OR REPLACE FILE FORMAT library_card_catalog.public.json_file_format
  ...
  STRIP_OUTER_ARRAY = TRUE  -- To load JSON array elements as separate rows
...
ON_ERROR = 'CONTINUE';  -- Optionally specify how to handle errors
...
SELECT RAW_STATUS:entities:hashtags -- hash tag is an array
FROM TWEET_INGEST;
